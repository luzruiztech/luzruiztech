# Hi, I'm Luz Emily Ruiz 👋
### IT Analyst | Cloud Support Engineer in Training | AWS | AI & Automation

📍 Buenos Aires, Argentina &nbsp;|&nbsp; 📧 lruizn.88@gmail.com &nbsp;|&nbsp; 🔗 [LinkedIn](https://linkedin.com/in/luzruizn)

---

## 👩‍💻 About Me

IT professional with **6+ years of experience** in functional-technical support and systems analysis, specialized in the healthcare sector (HIS/ERP). Currently transitioning into **Cloud Computing** with a focus on **AWS**, building hands-on skills through labs, certifications and real-world projects.

Passionate about support excellence, process automation, and the intersection of **Cloud + AI**.

---

## 🚀 Currently

- ☁️ Studying **AWS** — Cloud Practitioner path & hands-on labs
- 🤖 Building skills in **Cloud Support Engineering with AI/Automation**
- 🧪 Documenting practical labs in this GitHub portfolio
- 🎯 Goal: **AWS Certified Cloud Practitioner → SysOps Administrator**

---

## 🛠️ Tech Stack

**Cloud**
![AWS](https://img.shields.io/badge/AWS-232F3E?style=flat&logo=amazon-aws&logoColor=white)

**Databases**
![SQL Server](https://img.shields.io/badge/SQL_Server-CC2927?style=flat&logo=microsoft-sql-server&logoColor=white)
![Oracle](https://img.shields.io/badge/Oracle-F80000?style=flat&logo=oracle&logoColor=white)
![MySQL](https://img.shields.io/badge/MySQL-4479A1?style=flat&logo=mysql&logoColor=white)

**ITSM & Management**
![ServiceNow](https://img.shields.io/badge/ServiceNow-00C244?style=flat&logo=servicenow&logoColor=white)
![Jira](https://img.shields.io/badge/Jira-0052CC?style=flat&logo=jira&logoColor=white)
![Zendesk](https://img.shields.io/badge/Zendesk-03363D?style=flat&logo=zendesk&logoColor=white)

**Testing & APIs**
![Postman](https://img.shields.io/badge/Postman-FF6C37?style=flat&logo=postman&logoColor=white)
![Selenium](https://img.shields.io/badge/Selenium-43B02A?style=flat&logo=selenium&logoColor=white)

**Languages**
![Python](https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white)
![SQL](https://img.shields.io/badge/SQL-4479A1?style=flat&logo=postgresql&logoColor=white)

**Methodologies**
![Scrum](https://img.shields.io/badge/Scrum-6DB33F?style=flat)
![Agile](https://img.shields.io/badge/Agile-0052CC?style=flat)
![Kanban](https://img.shields.io/badge/Kanban-FF6C37?style=flat)

---

## 📂 Featured Projects

> 🔬 Hands-on AWS labs documented with screenshots, configurations and learnings.

| Repo | Descripción | Tecnologías |
|---|---|---|
| [lab-spl-200-intro-amazon-ec2](https://github.com/luzruiztech/lab-spl-200-intro-amazon-ec2) | Launch, monitor, resize and terminate an EC2 instance | EC2, CloudWatch, Security Groups, EBS |
| [DentiSalud](https://github.com/Luz-88/Proyecto-Final-CoderHouse_Luz_Ruiz) | Relational database system for dental clinic appointment management — triggers, stored procedures, roles & Docker | MySQL, Docker, Shell Scripting |
| *(coming soon)* | Next AWS lab | AWS |

---

## 🏅 Certifications

- ☁️ **AWS Cloud Practitioner Essentials** — Amazon Web Services (Jan 2026)
- ☁️ **AWS Technical Essentials** — Amazon Web Services (Feb 2026)
- 🔌 **Postman API Fundamentals Student Expert** — Badgr (Feb 2025)
- 🗄️ **SQL Course** — Coderhouse (Feb 2024)
- 🔄 **Scrum Foundation Professional Certificate (SFPC)** — CertiProf (Sep 2021)
- 📦 **Product Owner** — UTN FRBA (Jun 2021)
- 🧪 **Software Tester** — UTN (2020)
- 📊 **Business Intelligence Analyst V2** — UTN (2020)
- 🐍 **Python Essential** — LinkedIn Learning (2021–2022)

---

## 📊 GitHub Stats

![Luz's GitHub stats](https://github-readme-stats.vercel.app/api?username=luzruiztech&show_icons=true&theme=dark&hide_border=true)

---

## 🌐 Let's Connect

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=flat&logo=linkedin&logoColor=white)](https://linkedin.com/in/luzruizn)
[![Email](https://img.shields.io/badge/Email-D14836?style=flat&logo=gmail&logoColor=white)](mailto:lruizn.88@gmail.com)

---

*"Support is not just fixing problems — it's preventing them."* ☁️

---

<details>
<summary>🇦🇷 Versión en Español — clic para desplegar</summary>

<br>

# Hola, soy Luz Emily Ruiz 👋
### Analista IT | Cloud Support Engineer en Formación | AWS | IA & Automatización

📍 Buenos Aires, Argentina &nbsp;|&nbsp; 📧 lruizn.88@gmail.com &nbsp;|&nbsp; 🔗 [LinkedIn](https://linkedin.com/in/luzruizn)

---

## 👩‍💻 Sobre mí

Profesional IT con **más de 6 años de experiencia** en soporte técnico-funcional y análisis de sistemas, especializada en el sector sanitario (HIS/ERP). Actualmente en transición hacia **Cloud Computing** con foco en **AWS**, desarrollando habilidades prácticas a través de laboratorios, certificaciones y proyectos reales.

Apasionada por la excelencia en soporte, la automatización de procesos y la intersección entre **Cloud + IA**.

---

## 🚀 Actualmente

- ☁️ Estudiando **AWS** — camino a Cloud Practitioner y labs prácticos
- 🤖 Desarrollando habilidades en **Cloud Support Engineering con IA/Automatización**
- 🧪 Documentando laboratorios prácticos en este portfolio de GitHub
- 🎯 Objetivo: **AWS Certified Cloud Practitioner → SysOps Administrator**

---

## 📂 Proyectos Destacados

| Repo | Descripción | Tecnologías |
|---|---|---|
| [lab-spl-200-intro-amazon-ec2](https://github.com/luzruiztech/lab-spl-200-intro-amazon-ec2) | Lanzamiento, monitoreo, redimensionamiento y terminación de una instancia EC2 | EC2, CloudWatch, Security Groups, EBS |
| [DentiSalud](https://github.com/Luz-88/Proyecto-Final-CoderHouse_Luz_Ruiz) | Base de datos relacional para gestión de citas odontológicas — triggers, stored procedures, roles y Docker | MySQL, Docker, Shell Scripting |
| *(próximamente)* | Próximo lab AWS | AWS |

---

## 🏅 Certificaciones

- ☁️ **AWS Cloud Practitioner Essentials** — Amazon Web Services (Ene 2026)
- ☁️ **AWS Technical Essentials** — Amazon Web Services (Feb 2026)
- 🔌 **Postman API Fundamentals Student Expert** — Badgr (Feb 2025)
- 🗄️ **Curso de SQL** — Coderhouse (Feb 2024)
- 🔄 **Scrum Foundation Professional Certificate (SFPC)** — CertiProf (Sep 2021)
- 📦 **Product Owner** — UTN FRBA (Jun 2021)
- 🧪 **Testeador de Software** — UTN (2020)
- 📊 **Analista Business Intelligence V2** — UTN (2020)
- 🐍 **Python Esencial** — LinkedIn Learning (2021–2022)

---

## 🌐 Conectemos

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=flat&logo=linkedin&logoColor=white)](https://linkedin.com/in/luzruizn)
[![Email](https://img.shields.io/badge/Email-D14836?style=flat&logo=gmail&logoColor=white)](mailto:lruizn.88@gmail.com)

---

*"El soporte no es solo resolver problemas — es prevenirlos."* ☁️

</details>
